...
// FIX 1: Accept deps array so animations re-run correctly when slug changes
function useGSAP(cb, deps = []) {
  useEffect(() => {
    ...
    if (existing) {
      // FIX 2: If GSAP already loaded (e.g. slug change), run immediately
      run();
    } else {
...
export default function WorkDetail() {
  ...
  // Scroll to top immediately whenever the project slug changes
  useEffect(() => {
    window.scrollTo({ top: 0, left: 0, behavior: "instant" });
  }, [slug]);

  // FIX 3: Reset title visibility BEFORE GSAP fires, not in a separate useEffect
  // that can race with the animation. We do it synchronously inside useGSAP.
  useGSAP(
    (gsap, ST) => {
      // Kill any lingering ScrollTriggers from previous page
      ST.getAll().forEach((t) => t.kill());

      // Clear any pending safety timer from previous render
      if (safetyTimerRef.current) clearTimeout(safetyTimerRef.current);

      // FIX 4: Set initial state here (not in a separate useEffect) so GSAP
      // controls the element from the start — no flicker or competing styles.
      ...
      // Safety net — force visible after 2s if GSAP somehow stalls
      ...
      // FIX 5: Proper cleanup — kill all ST instances AND clear safety timer
      return () => {
        ST.getAll().forEach((t) => t.kill());
        if (safetyTimerRef.current) clearTimeout(safetyTimerRef.current);
      };
    },
    // FIX 6: Re-run animations when slug changes (different project)
    [slug]
  );

  // FIX 7: Use useCallback for navigate handler to avoid stale closures
  // No need to manually scroll — the useEffect above handles it on slug change
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* FIX 8: img-hover was applying transform AND grayscale together which
           conflicted with GSAP's scale animation on .gsap-img parent.
           Split into separate properties and use will-change for GPU compositing. */
        .img-hover {
          filter: grayscale(75%);
        ...
        .next-hover:hover .next-title {
          transform: scale(1.05) translateY(-4px);
        }

        /* FIX 9: next-hover background image transition — moved to CSS class
           so it works without inline onMouseEnter on the <img> itself */
        .next-hover .next-bg {
          opacity: 0.15;
          filter: grayscale(70%);
          ...
          <div>
            ...
            <div className="gsap-stagger" style={{ marginBottom: "3rem" }}>
              <Link to="/works" style={{ textDecoration: "none" }}>
                ...
                >
                  {/* FIX 10: The rotated div looked like a diamond/square, not an arrow.
                      Replaced with a proper left-arrow SVG chevron. */}
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
            ...
            <div
              style={{
                overflow: "hidden", // clip the GSAP slide-up
                marginBottom: "1.5rem",
                paddingBottom: "0.3em",
                paddingTop: "0.1em",
              }}
            >
              <h1
                ...
                style={{
                  ...
                  // FIX 11: Remove inline opacity/transform — GSAP owns these now
                  // (previously a competing useEffect set them, causing flicker)
                }}
              >
                ...
                {project.members.map((m) => (
                  ...
                  >
                    <div>
                      ...
                      <p
                        className="font-mono"
                        style={{ fontSize: "0.65rem", color: "rgba(240,237,232,0.3)", marginTop: 2 }}
                      >
                        {m.handle}
                      </p>
                    </div>
...
{project.videoUrl && (
  ...
  >
    ...
    <div style={{ position: "absolute", bottom: "1.2rem", left: "1.4rem" }}>
      ...
      >
        {/* FIX 9: bg image transition now handled by CSS class .next-bg / .next-hover:hover .next-bg
            instead of inline onMouseEnter on the <img>, which prevented the parent hover from working */}
        <img
        ...
        <div
          style={{
            ...
            alignItems: "center",
            justifyContent: "center",
            padding: "clamp(6rem, 18vw, 14rem) 2rem",
            textAlign: "center",
          }}
        >
          <span
          ...
          <h2
            className="next-title font-display"
            style={{
